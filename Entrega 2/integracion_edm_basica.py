...
a = sp.arange(0,2*sp.pi, .01)
x_at = atmosfera * sin(a)
y_at = atmosfera * cos(a)

tierra = plt.Circle((0,0), rtierra, color = 'cyan', label = "Superficie de la tierra")
plt.gcf().gca().add_artist(tierra)

plt.plot(x_at, y_at, color = 'red' , label = "Rango atmosfera")

plt.plot(x,y, label = "Orbita satelite A", color = 'black')


plt.title("Satélite Sentinel 1A")
plt.xlabel("X (m)")
plt.ylabel("Y (m)")
plt.grid(True)
